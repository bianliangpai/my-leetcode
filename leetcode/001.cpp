...
//public:
//
//	void QuickSort(vector<int>::iterator x, int numsize, int start)
//	{
//		if (numsize > 1)
//		{
//			int i = 0, j = numsize - 1;
...
//	}
//
//	int findnum(vector<int>& nums, int num, int start) {
//		int b = start; int e = nums.size() - 1;
//		while (b < e) {
//			if (num == nums[b]) return b + 1;
//			if (num == nums[e]) return e + 1;
//
//			if (num >((nums[b] + nums[e]) / 2)) b = (e + b) / 2 + 1;
//			else e = (e + b) / 2;
//		}
//		return 0;
//	}
...
//			position[nums[i]] = i;
//		for (int i = 0; i < nums.size(); i++)
//			if (position.end() != position.find(target - nums[i]) && i != position[target - nums[i]])
